...
public:
	...
	// Set関数
	void SetNumVertex(int nNumVertex) { m_nNumVertex = nNumVertex; }												// 頂点数
	void SetNumIndex(int nNumIndex) { m_nNumIndex = nNumIndex; }													// インデックス数
	void SetNumPolygon(int nNumPolygon) { m_nNumPolygon = nNumPolygon; }											// ポリゴン数
	void SetCylinderNum(CYLINDER CylinderNum, MESH_TYPE typeNum) { m_Mesh3D[typeNum].CylinderNum = CylinderNum; }	// 何角形か設定する
	void BindIdxBuff(LPDIRECT3DINDEXBUFFER9 IdxBuff) { m_pIdxBuff = IdxBuff; }										// バッファ番号の設定
	void BindMtxWorld(D3DXMATRIX mtxWorld) { m_mtxWorld = mtxWorld; }												// マトリクス情報
	void SetOneSize(D3DXVECTOR2 OneSize) { m_OneSize = OneSize; }													// 一つのブロックの大きさ
	void SetMeshNum(MESH_TYPE MeshNum) { m_MeshNum = MeshNum; }														// メッシュの番号の設定
